...
class TestRetryQueryOnDeadlock(IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self._operr_error_deadlock = OperationalError("", "", orig=MagicMock(args=[1213]))
        self._operr_error_other_error = OperationalError("", "", orig=MagicMock(args=[1000]))
    ...
    async def test_retry_on_deadlock_after_too_many_attempts(self) -> None:
        self._conn.execute.side_effect = self._operr_error_deadlock
        ...
        self.assertEqual(e.exception, self._operr_error_deadlock)
    ...
    async def test_retry_on_deadlock_unknown_error(self) -> None:
        self._conn.execute.side_effect = self._operr_error_other_error
        with self.assertRaises(OperationalError) as e:
            await self._run(
                maximum_number_of_retries=8,
                expected_number_of_execute_awaits=0,
            )
        self.assertEqual(e.exception, self._operr_error_other_error)

    async def test_retry_on_deadlock_unknown_error_after_tries(self) -> None:
        self._conn.execute.side_effect = [self._operr_error_deadlock for _ in range(5)] + [
            self._operr_error_other_error
        ]
        with self.assertRaises(OperationalError) as e:
            await self._run(
                maximum_number_of_retries=8,
    ...
    async def test_retry_on_deadlock_inside_number_of_errors_does_not_pass_the_limit(self) -> None:
        self._conn.execute.side_effect = [self._operr_error_deadlock for _ in range(5)] + [self._result_mock]
        await self._run(
            maximum_number_of_retries=8,
